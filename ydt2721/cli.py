...
def build_calculation_input(config: dict, target_margin: float = None) -> CalculationInput:
    """从配置字典构建统一计算输入对象

    所有 config 键 → CalculationInput 的映射集中在此处，
    避免散落在各处的 kwargs 映射与配置文件键名漂移。
    """
    sat = config.get('satellite', {})
    ...
    system = config.get('system', {})
    interference = config.get('interference') or {}

    return CalculationInput(
        satellite=SatelliteParams(
            longitude=sat.get('longitude', 0),
            eirp_ss=sat.get('eirp_ss', 0),
            gt_s=sat.get('gt_s', 0),
            gt_s_ref=sat.get('gt_s_ref', 0),
            sfd_ref=sat.get('sfd_ref', 0),
            bo_i=sat.get('bo_i', 6),
            bo_o=sat.get('bo_o', 3),
            transponder_bw=sat.get('transponder_bw', 54000000),
            adj_sat_longitude=sat.get('adj_sat_longitude'),
        ),
        carrier=CarrierParams(
            info_rate=carrier.get('info_rate', 2000000),
            fec_rate=carrier.get('fec_rate', 0.75),
            fec_type=carrier.get('fec_type', 'LDPC'),
            spread_gain=carrier.get('spread_gain', 1),
            modulation=carrier.get('modulation', 'QPSK'),
            modulation_index=carrier.get('modulation_index', 2),
            ebno_threshold=carrier.get('ebno_threshold', 4.5),
            alpha1=carrier.get('alpha1', 1.2),
            alpha2=carrier.get('alpha2', 1.4),
        ),
        tx_station=EarthStationParams(
            name=tx.get('name', '发射站'),
            longitude=tx.get('longitude', 0),
            latitude=tx.get('latitude', 0),
            antenna_diameter=tx.get('antenna_diameter', 4.5),
            efficiency=tx.get('efficiency', 0.65),
            frequency=tx.get('frequency', 14.25),
            polarization=tx.get('polarization', 'V'),
            feed_loss=tx.get('feed_loss', 1.5),
            antenna_noise_temp=tx.get('antenna_noise_temp', 35),
            receiver_noise_temp=tx.get('receiver_noise_temp', 75),
            upc_max_comp=tx.get('upc_max_comp', 5.0),
            loss_at=tx.get('loss_at', 0.5),
            loss_ar=tx.get('loss_ar', 0.5),
            hpa_bo=tx.get('hpa_bo', 3.0),
        ),
        rx_station=EarthStationParams(
            name=rx.get('name', '接收站'),
            longitude=rx.get('longitude', 0),
            latitude=rx.get('latitude', 0),
            antenna_diameter=rx.get('antenna_diameter', 1.8),
            efficiency=rx.get('efficiency', 0.65),
            frequency=rx.get('frequency', 12.50),
            polarization=rx.get('polarization', 'H'),
            feed_loss=rx.get('feed_loss', 0.2),
            antenna_noise_temp=rx.get('antenna_noise_temp', 35),
            receiver_noise_temp=rx.get('receiver_noise_temp', 75),
            upc_max_comp=tx.get('upc_max_comp', 5.0),
            loss_at=rx.get('loss_at', 0.5),
            loss_ar=rx.get('loss_ar', 0.5),
            hpa_bo=tx.get('hpa_bo', 3.0),
        ),
        uplink_availability=system.get('uplink_availability', 99.9),
        downlink_availability=system.get('downlink_availability', 99.9),
        target_margin=target_margin if target_margin is not None else system.get('target_margin', 0.0),
...
def execute_calculation(config: dict, output_prefix: str, output_format: str, print_json: bool = False,
    ...
    print("\n正在执行链路计算...")
    print(f"降雨衰减模型: ITU-Rpy")
    calc_input = build_calculation_input(config, target_margin)
    result = complete_link_budget_from_input(calc_input)
